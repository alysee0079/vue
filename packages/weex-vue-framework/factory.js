...
module.exports = function weexFactory(exports, document) {
  ...
  function initVirtualComponent(options) {
    ...
    var params = typeof data === "function" ? getData(data, vm) : data || {};
  ...
  function makeClassList(data) {
    ...
    if (Array.isArray(dataClass)) {
      ...
      classList.push.apply(
        classList,
        Object.keys(dataClass).filter(function (className) {
  ...
  function add$1(event, handler, capture, passive, params) {
    if (capture) {
  ...
  function isSimpleSpan(vnode) {
    return (
      vnode.children && vnode.children.length === 1 && !vnode.children[0].tag
    );
  }
